...
var argv = require('yargs')
  .usage('USAGE: scullog [-s <service>] [-i <ip>] [-p <port>] [-d <directory>] [-c <config>]')
  .options({
    's': {
      alias: 'service',
...
class Scullog {
  ...
  __initServer(resolve) {
    ...
    global.C.logger.info('listening on ' + this.conf.host + ':' + this.conf.port + " on " + (this.conf.ssl ? "https" : "http"));
    server.listen(this.conf.port, this.conf.host, () => {
      resolve(this.conf.port, this.conf.host);
    });
